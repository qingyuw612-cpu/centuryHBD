...
(function() {
  ...
  function startHomeFireworks() {
    ...
    function spawn() {
      var x = Math.random()*c.width, y = Math.random()*c.height*0.3;
      var col = COL[Math.floor(Math.random()*COL.length)];
      for (var i=0;i<30;i++) {
        var a=Math.random()*Math.PI*2, s=1.5+Math.random()*3.5;
        fw.push({x:x,y:y,c:col,vx:Math.cos(a)*s,vy:Math.sin(a)*s,age:0,life:0.6+Math.random()*0.8,sz:1.5+Math.random()*2});
      }
    }
    function loop() {
