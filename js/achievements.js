...
(function() {
  ...
  const ACHIEVEMENTS = [
    ...
    // === Haircut ===
    { id:'hair_S',      emoji:'', text:'Tony亲传',   sub:'理发获得S级',       check:()=>{var s=STORE.getInt('haircut_score')||0;return s>=360;} },
    { id:'hair_D',      emoji:'', text:'除草机',     sub:'理发获得D级',       check:()=>{var s=STORE.getInt('haircut_score')||0;return s>0&&s<=100;} },
    { id:'hair_first',  emoji:'', text:'第一次上班', sub:'首次完成理发',      check:()=>STORE.getBool('haircut_complete') },
    { id:'hair_insult', emoji:'', text:'顾客已报警', sub:'累计被骂5次以上',   check:()=>(STORE.getInt('haircut_insults')||0)>=5 },
    { id:'hair_insult10',emoji:'',text:'投诉信堆成山',sub:'累计被骂10次以上', check:()=>(STORE.getInt('haircut_insults')||0)>=10 },
    { id:'hair_5x',     emoji:'', text:'敬业精神',   sub:'理发5次以上',       check:()=>(STORE.getInt('haircut_plays')||0)>=5 },
    ...
    // === Bodyguard ===
    { id:'bg_S',       emoji:'', text:'零失误',     sub:'上班零镜头事故',    check:()=>{var c=STORE.getInt('bodyguard_camera')||-1;var p=STORE.getInt('bodyguard_plays')||0;return p>0&&c===0;} },
    { id:'bg_first',   emoji:'', text:'第一天上班', sub:'首次完成上班',      check:()=>STORE.getBool('bodyguard_complete') },
    { id:'bg_camera5', emoji:'', text:'小红书素材', sub:'被拍到5次以上',     check:()=>(STORE.getInt('bodyguard_camera')||0)>=5 },
    { id:'bg_caught10',emoji:'', text:'接住了！',   sub:'单局接住比格10个以上',check:()=>(STORE.getInt('bodyguard_caught')||0)>=10 },
    ...
    { id:'meta_click',emoji:'', text:'咒语初学者',   sub:'成功召唤了一次字母环',  check:()=>{return STORE.getBool('click_ring_triggered');} },
  ];
  ...
  function processQueue() {
    ...
    showing = true;
    var ach = queue.shift();
    toastEl.innerHTML = '<div class="ach-toast-inner"><div><div class="ach-toast-title">成就解锁：'+ach.text+'</div><div class="ach-toast-sub">'+ach.sub+'</div></div></div>';
